    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-8">
        <select
          ...
          className="min-w-0 flex-1 sm:flex-none bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-[#d4af37]/50 cursor-pointer"
        >
      ...
      {mpReturn === 'connected' && (
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-green-400/30 bg-green-400/10 px-4 sm:px-5 py-4">
          <CheckCircle2 className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />
          <p className="min-w-0 text-xs sm:text-sm text-green-300 break-words">
      ...
      {mpReturn === 'error' && (
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-red-400/30 bg-red-400/10 px-4 sm:px-5 py-4">
          <AlertCircle className="w-4 h-4 text-red-400 mt-0.5 shrink-0" />
          <p className="min-w-0 text-xs sm:text-sm text-red-300 break-words">
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-3 sm:gap-4 rounded-2xl border border-white/10 bg-white/[0.03] px-4 sm:px-5 py-4">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${mpConnected ? 'bg-green-400/10 border-green-400/30' : 'bg-amber-400/10 border-amber-400/30'}`}>
            {mpConnected
              ? <CheckCircle2 className="w-[18px] h-[18px] text-green-400" />
              : <Link2 className="w-[18px] h-[18px] text-amber-400" />}
          </span>
          <div className="min-w-0">
            <p className="text-sm font-bold text-white truncate">Split automático do PIX</p>
            <p className="text-xs text-white/40 line-clamp-2 sm:truncate">
              {mpConnected
        ...
        {!mpConnected && (
          <button
            ...
            disabled={connecting}
            className="w-full sm:w-auto shrink-0 inline-flex items-center justify-center gap-2 rounded-xl bg-[#d4af37] px-4 py-2.5 text-sm font-bold text-black hover:bg-[#e5c14e] transition-colors disabled:opacity-60"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        ...
        <motion.div
          ...
          className="min-w-0 bg-gradient-to-br from-[#d4af37]/20 to-[#d4af37]/5 border border-[#d4af37]/30 rounded-2xl p-5 sm:p-6"
        >
          <div className="flex items-center gap-2.5 mb-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#d4af37]/20 border border-[#d4af37]/30">
              <TrendingUp className="w-[18px] h-[18px] text-[#d4af37]" />
            </span>
            <span className="min-w-0 break-words text-[10px] font-black uppercase tracking-[0.3em] text-[#d4af37]">Total do mês</span>
          </div>
          <p className="text-2xl lg:text-3xl font-serif font-bold text-white tabular-nums break-words">
        ...
        <motion.div
          ...
          className="min-w-0 bg-white/[0.04] border border-white/5 rounded-2xl p-5 sm:p-6"
        >
          <div className="flex items-center gap-2.5 mb-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-violet-400/10 border border-violet-400/20">
              <CreditCard className="w-[18px] h-[18px] text-violet-400" />
            </span>
            <span className="min-w-0 break-words text-[10px] font-black uppercase tracking-[0.3em] text-violet-400">Cartão</span>
          </div>
          <p className="text-xl lg:text-2xl font-serif font-bold text-white tabular-nums break-words">
            {isLoading ? <span className="opacity-40">…</span> : fmt(cardTotal)}
          </p>
          <p className="mt-1 text-xs text-white/30 break-words">{cardOrders.length} pedidos · coletado via Stripe</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="min-w-0 bg-white/[0.04] border border-white/5 rounded-2xl p-5 sm:p-6 sm:col-span-2 lg:col-span-1"
        >
          <div className="flex items-center gap-2.5 mb-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-green-400/10 border border-green-400/20">
              <Smartphone className="w-[18px] h-[18px] text-green-400" />
            </span>
            <span className="min-w-0 break-words text-[10px] font-black uppercase tracking-[0.3em] text-green-400">PIX</span>
          </div>
          <p className="text-xl lg:text-2xl font-serif font-bold text-white tabular-nums break-words">
            {isLoading ? <span className="opacity-40">…</span> : fmt(pixTotal)}
          </p>
          <p className="mt-1 text-xs text-white/30 break-words">
      ...
      {!isLoading && pixTotal > 0 && !mpConnected && (
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-green-400/20 bg-green-400/5 px-4 sm:px-5 py-4">
      ...
      {/* Tabela de pedidos */}
      <div className="min-w-0 bg-[#0f0f0f]/60 border border-white/5 rounded-2xl overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b border-white/5 flex flex-wrap items-center justify-between gap-2">
          <h2 className="min-w-0 break-words text-xs sm:text-sm font-bold text-white/60 uppercase tracking-widest">
            Detalhamento · {MONTHS[month].slice(0, 3)} {year}
          </h2>
          {!isLoading && orders.length > 0 && (
            <span className="shrink-0 whitespace-nowrap text-[10px] font-black text-[#d4af37] bg-[#d4af37]/10 border border-[#d4af37]/20 rounded-full px-3 py-1">
        ...
        {isLoading ? (
          <div className="p-8 sm:p-10 text-center text-white/30">Carregando…</div>
        ) : orders.length === 0 ? (
          <div className="p-8 sm:p-10 text-center text-white/30">Nenhuma comissão neste período.</div>
        ) : (
